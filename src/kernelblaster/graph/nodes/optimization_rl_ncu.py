...
async def optimization_rl_ncu(state: GraphState):
    ...
    Requires curated CUDA files (driver.cpp and final_cuda.cu) from data/kernelbench-cuda.
    ...
    curated_final_cuda_cu = curated_dir / "final_cuda.cu"

    # Fallbacks: allow using files that already exist in the run folder.
    run_driver_cpp = base_folder / "driver.cpp"
    run_final_cuda_cu = base_folder / "final_cuda.cu"
    
    # Handle missing cuda_fp - require curated or run-folder final_cuda.cu
    cuda_fp = state.get("cuda_fp")
    if cuda_fp is None:
        if curated_final_cuda_cu.exists():
            cuda_fp = curated_final_cuda_cu
            state["logger"].info(f"Using curated final_cuda.cu from {curated_dir} as cuda_fp: {cuda_fp}")
        elif run_final_cuda_cu.exists():
            cuda_fp = run_final_cuda_cu
            state["logger"].info(f"Using run-folder final_cuda.cu as cuda_fp: {cuda_fp}")
        else:
            state["logger"].error(
                f"No cuda_fp available. Required files not found:\n"
                f"  - Curated: {curated_final_cuda_cu}\n"
                f"  - Run folder: {run_final_cuda_cu}\n"
