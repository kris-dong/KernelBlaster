...
_RISCV_TECHNIQUE_MAP: Mapping[str, str] = {
    ...
    "2.3_rvv_vectorization": (
        "Vectorize the inner loop with V-extension intrinsics. This "
        "toolchain is RVV 1.0 — ALL intrinsics MUST be prefixed with "
        "`__riscv_` (unprefixed names fail with `implicit declaration`). "
        "Use `__riscv_vsetvl_e32m8(n - i)` to stripmine, then "
        "`__riscv_vle32_v_f32m8` / `__riscv_vse32_v_f32m8` for stride-1 "
        "load/store. CRITICAL: for common elementwise idioms, use the "
        "direct single-instruction form, NOT compare-then-merge — one "
        "vector op vs three: `__riscv_vfmax_vf_f32m8(v, 0.0f, vl)` for "
        "ReLU / max-with-scalar, `__riscv_vfmin_vf_f32m8` for clamp, "
        "`__riscv_vfabs_v_f32m8` for absolute value, "
        "`__riscv_vfsgnj_vv` for sign copy, `__riscv_vfmacc_vv` for "
        "fused-multiply-add, `__riscv_vfredosum_vs` / "
        "`__riscv_vfredmax_vs` for in-vector reductions. Prefer LMUL=m8 "
        "for pure-elementwise (widens the memory transaction); drop to "
        "m4/m2 under register pressure. Skip when the target lacks V — "
        "falls back to scalar cleanly."
    ),
