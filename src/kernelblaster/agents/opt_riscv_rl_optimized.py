...
class OptimizedRLRiscvAgent(RLOptimizedAgentBase):
    ...
    _RISCV_SYSTEM_PROMPT_TMPL = (
        ...
        "RVV idiom cheatsheet — this toolchain is RVV 1.0, so ALL "
        "intrinsics MUST be prefixed with `__riscv_` (e.g. "
        "`__riscv_vfmax_vf_f32m8`, NOT `vfmax_vf_f32m8`). Bare unprefixed "
        "names fail with `implicit declaration` at compile time. Use these "
        "one-op forms (not the mask+merge equivalents):\n"
        "- `__riscv_vfmax_vf_f32m8(v, 0.0f, vl)` for ReLU / max-with-"
        "scalar. Do NOT emit `__riscv_vmfgt_vf_*` + `__riscv_vmerge_vvm_*` "
        "for this — that's three vector ops where one suffices.\n"
        "- `__riscv_vfmin_vf_f32m8(v, cap, vl)` for clamp-max / ReLU6 "
        "upper bound; chain with vfmax_vf for two-sided clamp.\n"
        "- `__riscv_vfabs_v_f32m8(v, vl)` for absolute value.\n"
        "- `__riscv_vfsgnj_vv` / `_vfsgnjn_vv` / `_vfsgnjx_vv` for sign "
        "manipulation.\n"
        "- `__riscv_vfmacc_vv_f32m8(acc, a, b, vl)` (a*b + acc) for "
        "reductions / dot products.\n"
        "- `__riscv_vfredosum_vs` / `_vfredmax_vs` for in-vector "
        "reductions to a scalar (one instruction, LMUL-wide).\n"
        "- Stripmine with `size_t vl = __riscv_vsetvl_e32m8(n - i);` "
        "then `__riscv_vle32_v_f32m8` / `__riscv_vse32_v_f32m8` for "
        "stride-1 load/store.\n"
