...
static void swish_cpu(const cl_half* x, cl_half* out, int count) {
    for (int i = 0; i < count; i++) {
        float xf = half_to_float(x[i]);
        float sigmoid_x = 1.0f / (1.0f + expf(-xf));
        float result = xf * sigmoid_x;
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for swish, total=%d elements...\n", (int)total);
        swish_cpu(h_x, h_ref, total);
    ...
    cl_kernel kernel = clCreateKernel(program, "swish", &err);
    ...
    if (g_profile) { print_event_timing("swish", event); clReleaseEvent(event); }
