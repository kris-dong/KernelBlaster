...
static void softsign_cpu(const cl_half* x, cl_half* out, int count) {
    for (int i = 0; i < count; i++) {
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for softsign, total=%zu...\n", total);
        softsign_cpu(h_x, h_ref, total);
    ...
    cl_kernel kernel = clCreateKernel(program, "softsign", &err);
    ...
    if (g_profile) { print_event_timing("softsign", event); clReleaseEvent(event); }
