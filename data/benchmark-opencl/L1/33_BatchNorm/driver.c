...
static int load_reference(cl_half* data, size_t count, const char* path) {
    FILE* f = fopen(path, "rb");
    if (!f) return 0;
    size_t r = fread(data, sizeof(cl_half), count, f);
    fclose(f);
    return r == count;
}

static void batchnorm_cpu(
    const cl_half* x,
    ...
    for (int c = 0; c < C; c++) {
        ...
        for (int n = 0; n < N; n++)
            for (int hw = 0; hw < HW; hw++)
                sum += (double)half_to_float(x[(n * C + c) * HW + hw]);
        double mean = sum / NHW;

        double vsum = 0.0;
        for (int n = 0; n < N; n++)
            for (int hw = 0; hw < HW; hw++) {
                double d = (double)half_to_float(x[(n * C + c) * HW + hw]) - mean;
                vsum += d * d;
            }
        double var = vsum / NHW;
        float inv_std = 1.0f / sqrtf((float)var + EPS_F);
        float wc = half_to_float(weight[c]);
        float bc = half_to_float(bias[c]);

        for (int n = 0; n < N; n++)
            for (int hw = 0; hw < HW; hw++) {
                int idx = (n * C + c) * HW + hw;
                float xv = half_to_float(x[idx]);
                float yv = ((xv - (float)mean) * inv_std) * wc + bc;
                y[idx] = float_to_half(yv);
            }
    }
}
...
int main(int argc, char** argv) {
    ...
    int N = BATCH_SIZE, C = FEATURES, H = DIM1, W = DIM2;
    size_t total     = (size_t)N * C * H * W;
    size_t tot_bytes = total * sizeof(cl_half);

    cl_half* h_x      = (cl_half*)malloc(tot_bytes);
    cl_half* h_weight = (cl_half*)malloc(C * sizeof(cl_half));
    cl_half* h_bias   = (cl_half*)malloc(C * sizeof(cl_half));
    cl_half* h_y      = (cl_half*)malloc(tot_bytes);
    cl_half* h_ref    = (cl_half*)malloc(tot_bytes);

    fill_random_half(h_x,      (int)total, 42);
    fill_random_half(h_weight, C,          123);
    fill_random_half(h_bias,   C,          456);

    if (g_generate_reference) {
        printf("Computing CPU reference for BatchNorm2d...\n");
        batchnorm_cpu(h_x, h_weight, h_bias, h_ref, N, C, H, W);
        if (save_reference(h_ref, total, REFERENCE_FILE)) {
            printf("Reference saved to %s\n", REFERENCE_FILE);
            printf("passed\n");
        } else { printf("failed\n"); }
        free(h_x); free(h_weight); free(h_bias); free(h_y); free(h_ref);
        return 0;
    }

    if (!load_reference(h_ref, total, REFERENCE_FILE)) {
    ...
    /* ---------- OpenCL setup ---------- */
    cl_int err;
    cl_platform_id platform; cl_uint nplat;
    clGetPlatformIDs(1, &platform, &nplat);

    cl_device_id device; cl_uint ndev;
    err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, 1, &device, &ndev);
    if (err != CL_SUCCESS || ndev == 0)
        clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL, 1, &device, &ndev);
    ...
    /*
     * Single kernel: batchnorm
     * Args (7):
     *   0: x       __global const half*
     *   1: weight  __global const half*
     *   2: bias    __global const half*
     *   3: y       __global half*
     *   4: N       int
     *   5: H       int
     *   6: W       int
     * C is baked in as compile-time constant FEATURES=64
     */
    cl_kernel kernel = clCreateKernel(program, "batchnorm", &err);
    ...
    cl_mem d_weight = clCreateBuffer(ctx, CL_MEM_READ_ONLY  | CL_MEM_COPY_HOST_PTR,
                                     C * sizeof(cl_half), h_weight, &err);
    cl_mem d_bias   = clCreateBuffer(ctx, CL_MEM_READ_ONLY  | CL_MEM_COPY_HOST_PTR,
                                     C * sizeof(cl_half), h_bias,   &err);
    cl_mem d_y      = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY,
                                     tot_bytes,           NULL,     &err);
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: buffer creation (%d)\n", err); return 1; }

    int n_val = N, h_val = H, w_val = W;

    /* 7 args exactly matching kernel signature */
    ...
    clSetKernelArg(kernel, 5, sizeof(int),    &h_val);
    clSetKernelArg(kernel, 6, sizeof(int),    &w_val);

    /* One work-item per channel: global = C = 64, local = 1 */
    size_t global_work = (size_t)C;
    size_t local_work  = 1;
    ...
    /* Timed run */
    cl_event event;
    err = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &global_work, &local_work,
                                 0, NULL, g_profile ? &event : NULL);
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: enqueue (%d)\n", err); return 1; }
    clFinish(queue);

    if (g_profile) { print_event_timing("batchnorm", event); clReleaseEvent(event); }

    err = clEnqueueReadBuffer(queue, d_y, CL_TRUE, 0, tot_bytes, h_y, 0, NULL, NULL);
