...
static void smooth_l1_loss_cpu(const cl_half* predictions, const cl_half* targets,
                                cl_half* output, int total) {
    double sum = 0.0;
    ...
    output[0] = float_to_half((float)(sum / total));
}
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for Smooth L1 Loss...\n");
        smooth_l1_loss_cpu(h_predictions, h_targets, h_ref, input_elems);
    ...
    /* Single kernel: smooth_l1_loss */
    cl_kernel kernel = clCreateKernel(program, "smooth_l1_loss", &err);
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: clCreateKernel (%d)\n", err); return 1; }

    /* num_groups for the reduction — must match kernel launch */
    int num_groups = (TOTAL_ELEMENTS + 255) / 256;
    size_t partial_bytes = (size_t)num_groups * sizeof(cl_float);
    ...
    cl_mem d_partial = clCreateBuffer(ctx, CL_MEM_READ_WRITE,
    ...
    clSetKernelArg(kernel, 0, sizeof(cl_mem), &d_predictions);  /* predictions */
    clSetKernelArg(kernel, 1, sizeof(cl_mem), &d_targets);       /* targets     */
    clSetKernelArg(kernel, 2, sizeof(cl_mem), &d_partial);       /* partial     */
    clSetKernelArg(kernel, 3, sizeof(cl_mem), &d_output);        /* output      */
    clSetKernelArg(kernel, 4, sizeof(int),    &total_val);       /* total       */
    clSetKernelArg(kernel, 5, 256 * sizeof(cl_float), NULL);     /* lmem        */

    size_t global_work = (size_t)num_groups * 256;
    ...
    if (g_profile) { print_event_timing("smooth_l1_loss", event); clReleaseEvent(event); }
    ...
    fprintf(stderr, "Result: got=%f, ref=%f, abs_diff=%f, rel_diff=%f\n", got, ref, diff, rel);
