...
static void relu_cpu(const cl_half* input, cl_half* output, int count) {
    for (int i = 0; i < count; i++) {
        float v = half_to_float(input[i]);
        output[i] = float_to_half(v > 0.0f ? v : 0.0f);
    }
}
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference (fp16) for ReLU (batch=%d, dim=%d)...\n", BATCH_SIZE, DIM);
        relu_cpu(h_x, h_ref, total_elems);
    ...
    int n_val = (int)total_elems;
    clSetKernelArg(kernel, 0, sizeof(cl_mem), &d_x);
    clSetKernelArg(kernel, 1, sizeof(cl_mem), &d_out);
    clSetKernelArg(kernel, 2, sizeof(int), &n_val);

    size_t global_work[1] = {total_elems};
    ...
    if (g_profile) { print_event_timing("relu", event); clReleaseEvent(event); }
