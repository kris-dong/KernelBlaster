...
static void fill_random_half(cl_half* data, int count, unsigned int seed) {
    ...
    for (int i = 0; i < count; i++) {
        /* Use small values near 1.0 to avoid underflow/overflow in product */
        float v = ((float)rand() / RAND_MAX) * 0.4f + 0.8f;
        /* Randomly negate */
        if (rand() % 2) v = -v;
...
/* Product reduction over dim1: input (B, D1, D2) -> output (B, D2) */
static void prod_reduce_cpu(const cl_half* x, cl_half* out,
                             int batch, int d1, int d2) {
    for (int b = 0; b < batch; b++) {
        for (int j = 0; j < d2; j++) {
            float prod = 1.0f;
            for (int i = 0; i < d1; i++) {
                prod *= half_to_float(x[b * d1 * d2 + i * d2 + j]);
            }
            out[b * d2 + j] = float_to_half(prod);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        ...
        prod_reduce_cpu(h_x, h_ref, BATCH_SIZE, DIM1, DIM2);
    ...
    cl_kernel kernel = clCreateKernel(program, "prod_reduce", &err);
    ...
    clSetKernelArg(kernel, 3, sizeof(int),    &d1_val);
    clSetKernelArg(kernel, 4, sizeof(int),    &d2_val);
    ...
    if (g_profile) { print_event_timing("prod_reduce", event); clReleaseEvent(event); }
