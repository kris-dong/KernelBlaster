...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference (fp16) for M=%d N=%d...\n", M, N);
        for (size_t i = 0; i < mat_elems; i++) {
            float a = half_to_float(h_A[i]);
            h_ref[i] = float_to_half(a * s_val);
        }
    ...
    cl_kernel kernel = clCreateKernel(program, "scalar_mul", &err);
    ...
    int total = (int)mat_elems;
    clSetKernelArg(kernel, 0, sizeof(cl_mem), &d_A);
    clSetKernelArg(kernel, 1, sizeof(cl_mem), &d_C);
    clSetKernelArg(kernel, 2, sizeof(float), &s_val);
    clSetKernelArg(kernel, 3, sizeof(int), &total);

    size_t global_work[1] = {(size_t)mat_elems};
    ...
    if (g_profile) { print_event_timing("scalar_mul", event); clReleaseEvent(event); }
    ...
    clReleaseMemObject(d_A); clReleaseMemObject(d_C);
