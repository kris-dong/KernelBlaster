...
static void sigmoid_cpu(const cl_half* x, cl_half* out, int count) {
    for (int i = 0; i < count; i++) {
        float val = half_to_float(x[i]);
        float s = 1.0f / (1.0f + expf(-val));
        out[i] = float_to_half(s);
    }
}
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference sigmoid for %d elements...\n", (int)total);
        sigmoid_cpu(h_x, h_ref, total);
    ...
    cl_kernel kernel = clCreateKernel(program, "sigmoid_kernel", &err);
    ...
    if (g_profile) { print_event_timing("sigmoid_kernel", event); clReleaseEvent(event); }
