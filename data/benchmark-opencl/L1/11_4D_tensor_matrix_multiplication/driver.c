...
/* A: (B_DIM, I_DIM, J_DIM, L_DIM)
   B: (L_DIM, K_DIM)
   C: (B_DIM, I_DIM, J_DIM, K_DIM)
...
/* CPU reference: C[b,i,j,k] = sum_l A[b,i,j,l] * B[l,k]
   A layout: [b][i][j][l] -> index = b*(I_DIM*J_DIM*L_DIM) + i*(J_DIM*L_DIM) + j*L_DIM + l
   B layout: [l][k]       -> index = l*K_DIM + k
   C layout: [b][i][j][k] -> index = b*(I_DIM*J_DIM*K_DIM) + i*(J_DIM*K_DIM) + j*K_DIM + k
*/
static void tensor_matmul_cpu(const cl_half* A, const cl_half* B, cl_half* C) {
            ...
            for (int j = 0; j < J_DIM; j++) {
                for (int k = 0; k < K_DIM; k++) {
                    ...
                    for (int l = 0; l < L_DIM; l++) {
                        int a_idx = b*(I_DIM*J_DIM*L_DIM) + i*(J_DIM*L_DIM) + j*L_DIM + l;
                        int b_idx = l*K_DIM + k;
                        sum += half_to_float(A[a_idx]) * half_to_float(B[b_idx]);
                    }
                    int c_idx = b*(I_DIM*J_DIM*K_DIM) + i*(J_DIM*K_DIM) + j*K_DIM + k;
                    C[c_idx] = float_to_half(sum);
                }
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    size_t a_elems = (size_t)B_DIM * I_DIM * J_DIM * L_DIM;
    size_t b_elems = (size_t)L_DIM * K_DIM;
    size_t c_elems = (size_t)B_DIM * I_DIM * J_DIM * K_DIM;
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference (fp16)...\n");
        tensor_matmul_cpu(h_A, h_B, h_ref);
    ...
    cl_kernel kernel = clCreateKernel(program, "tensor_matmul", &err);
    ...
    int b_dim = B_DIM, i_dim = I_DIM, j_dim = J_DIM, l_dim = L_DIM, k_dim = K_DIM;
    clSetKernelArg(kernel, 0, sizeof(cl_mem), &d_A);
    clSetKernelArg(kernel, 1, sizeof(cl_mem), &d_B);
    clSetKernelArg(kernel, 2, sizeof(cl_mem), &d_C);
    clSetKernelArg(kernel, 3, sizeof(int), &b_dim);
    clSetKernelArg(kernel, 4, sizeof(int), &i_dim);
    clSetKernelArg(kernel, 5, sizeof(int), &j_dim);
    clSetKernelArg(kernel, 6, sizeof(int), &l_dim);
    clSetKernelArg(kernel, 7, sizeof(int), &k_dim);

    /* global: (k_dim, j_dim, b_dim * i_dim) */
    size_t global_work[3] = {(size_t)K_DIM, (size_t)J_DIM, (size_t)B_DIM * I_DIM};
    ...
    if (g_profile) { print_event_timing("tensor_matmul", event); clReleaseEvent(event); }
    ...
    for (size_t idx = 0; idx < c_elems; idx++) {
        float got = half_to_float(h_C[idx]);
