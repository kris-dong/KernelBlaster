...
static void log_softmax_cpu(const cl_half* x, cl_half* out, int batch, int dim) {
    for (int b = 0; b < batch; b++) {
        const cl_half* row = x + b * dim;
        cl_half* out_row = out + b * dim;

        // Find max
        float max_val = half_to_float(row[0]);
        for (int i = 1; i < dim; i++) {
            float v = half_to_float(row[i]);
        ...
        float sum = 0.0f;
        for (int i = 0; i < dim; i++) {
            sum += expf(half_to_float(row[i]) - max_val);
        }
        float log_sum = logf(sum);

        // log_softmax = x - max - log(sum)
        for (int i = 0; i < dim; i++) {
            float val = half_to_float(row[i]) - max_val - log_sum;
            out_row[i] = float_to_half(val);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference (log_softmax) for batch=%d, dim=%d...\n", BATCH_SIZE, DIM);
        log_softmax_cpu(h_x, h_ref, BATCH_SIZE, DIM);
    ...
    cl_kernel kernel = clCreateKernel(program, "log_softmax", &err);
    ...
    if (g_profile) { print_event_timing("log_softmax", event); clReleaseEvent(event); }
