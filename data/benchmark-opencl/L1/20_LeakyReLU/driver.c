...
#define DIM 16384
#define TOTAL_ELEMS (BATCH_SIZE * DIM)
...
static void leaky_relu_cpu(const cl_half* x, cl_half* out, int count, float negative_slope) {
    for (int i = 0; i < count; i++) {
        float val = half_to_float(x[i]);
        float result = (val >= 0.0f) ? val : val * negative_slope;
        out[i] = float_to_half(result);
    }
}
...
int main(int argc, char** argv) {
    ...
    size_t total_elems = TOTAL_ELEMS;
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference (fp16) for LeakyReLU...\n");
        leaky_relu_cpu(h_x, h_ref, total_elems, negative_slope);
    ...
    cl_kernel kernel = clCreateKernel(program, "leaky_relu", &err);
    ...
    size_t global_work = (total_elems + 255) / 256 * 256;
    ...
    if (g_profile) { print_event_timing("leaky_relu", event); clReleaseEvent(event); }
