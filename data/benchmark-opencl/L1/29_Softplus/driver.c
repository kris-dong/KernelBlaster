...
static void softplus_cpu(const cl_half* x, cl_half* out, int count) {
    for (int i = 0; i < count; i++) {
        float val = half_to_float(x[i]);
        /* softplus(x) = log(1 + exp(x)) */
        float result = log1pf(expf(val));
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for softplus, total=%zu...\n", total);
        softplus_cpu(h_x, h_ref, (int)total);
    ...
    cl_kernel kernel = clCreateKernel(program, "softplus", &err);
    ...
    if (g_profile) { print_event_timing("softplus", event); clReleaseEvent(event); }
