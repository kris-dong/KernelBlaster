...
#define INPUT_ELEMS  (BATCH_SIZE * DIM1 * DIM2)
#define OUTPUT_ELEMS (BATCH_SIZE * 1 * DIM2)
...
/* CPU reference: sum reduction over dim=1 (DIM1), keepdim=True
   Input shape:  (BATCH_SIZE, DIM1, DIM2)
   Output shape: (BATCH_SIZE, 1, DIM2)
*/
static void sum_reduce_cpu(const cl_half* x, cl_half* out) {
    for (int b = 0; b < BATCH_SIZE; b++) {
        for (int j = 0; j < DIM2; j++) {
            ...
            out[b * DIM2 + j] = float_to_half(sum);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    fill_random_half(h_x, INPUT_ELEMS, 42);

    if (g_generate_reference) {
        printf("Computing CPU reference (fp16)...\n");
        sum_reduce_cpu(h_x, h_ref);
    ...
    cl_kernel kernel = clCreateKernel(program, "sum_reduce_dim1", &err);
    ...
    if (g_profile) { print_event_timing("sum_reduce_dim1", event); clReleaseEvent(event); }
