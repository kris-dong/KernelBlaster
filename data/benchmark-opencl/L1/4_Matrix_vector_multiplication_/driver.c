...
/* Helper to fill a buffer of cl_half with deterministic random values */
...
/* CPU reference for matrix-vector multiplication: C = A * B
   A: (M x K) flattened row-major, B: (K x 1), C: (M x 1) */
static void matvec_cpu_half(const cl_half* A, const cl_half* B, cl_half* C) {
    for (int i = 0; i < M; i++) {
        float sum = 0.0f;
        const cl_half* a_row = A + ((size_t)i * K);
        for (int k = 0; k < K; k++) {
            sum += half_to_float(a_row[k]) * half_to_float(B[k]);
        }
        C[i] = float_to_half(sum);
    }
}
...
int main(int argc, char** argv) {
    ...
    size_t B_elems = (size_t)K;
    size_t C_elems = (size_t)M;
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference (fp16) for M=%d K=%d...\n", M, K);
        matvec_cpu_half(h_A, h_B, h_ref);
    ...
    if (!ref_loaded) {
        if (g_profile) fprintf(stderr, "WARNING: No cached reference found, computing CPU reference...\n");
        matvec_cpu_half(h_A, h_B, h_ref);
    }
    ...
    cl_kernel kernel = clCreateKernel(program, "matvec", &err);
    ...
    /* Set kernel arguments in the exact order expected by kernel.cl:
    ...
    */
    ...
    err |= clSetKernelArg(kernel, 3, sizeof(int), &m_val);
    err |= clSetKernelArg(kernel, 4, sizeof(int), &k_val);
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: clSetKernelArg (%d)\n", err); goto cleanup; }

    /* global_work: {cols, rows} => cols = 1, rows = M */
    size_t global_work[2] = {1, M};
    /* local_work: ensure divisibility; use {1,16} */
    size_t local_work[2] = {1, 16};
    ...
    if (g_profile && event) { print_event_timing("matvec", event); clReleaseEvent(event); event = NULL; }
