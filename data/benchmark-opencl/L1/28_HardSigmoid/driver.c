...
static void fill_random_half(cl_half* data, int count, unsigned int seed) {
    ...
    for (int i = 0; i < count; i++) {
        /* randn approximation using Box-Muller */
        float u1 = ((float)rand() / RAND_MAX) + 1e-7f;
        float u2 = ((float)rand() / RAND_MAX);
        float z = sqrtf(-2.0f * logf(u1)) * cosf(2.0f * 3.14159265f * u2);
        data[i] = float_to_half(z);
    }
}

static void hardsigmoid_cpu(const cl_half* x, cl_half* y, int count) {
    for (int i = 0; i < count; i++) {
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for HardSigmoid (total=%d)...\n", (int)total);
        hardsigmoid_cpu(h_x, h_ref, (int)total);
    ...
    int ref_loaded = load_reference(h_ref, (int)total, REFERENCE_FILE);
    if (!ref_loaded) {
    ...
    cl_kernel kernel = clCreateKernel(program, "hardsigmoid", &err);
    ...
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: d_y buffer creation (%d)\n", err); return 1; }
    ...
    if (g_profile) { print_event_timing("hardsigmoid", event); clReleaseEvent(event); }
    ...
    else printf("failed (%d mismatches, max_abs_err=%f)\n", errors, max_err);
