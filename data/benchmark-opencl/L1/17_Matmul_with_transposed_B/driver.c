...
*/
static void matmul_transB_cpu(const cl_half* A, const cl_half* B, cl_half* C,
                               int m, int k, int n) {
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            float sum = 0.0f;
            for (int kk = 0; kk < k; kk++) {
                sum += half_to_float(A[i * k + kk]) * half_to_float(B[j * k + kk]);
            }
...
int main(int argc, char** argv) {
    ...
    size_t b_elems = (size_t)N_DIM * K_DIM;
    ...
    if (g_generate_reference) {
        ...
        matmul_transB_cpu(h_A, h_B, h_ref, M_DIM, K_DIM, N_DIM);
    ...
    cl_kernel kernel = clCreateKernel(program, "matmul_transB", &err);
    ...
    /* global: (N_DIM, M_DIM), local: (16, 16) */
    ...
    if (g_profile) { print_event_timing("matmul_transB", event); clReleaseEvent(event); }
