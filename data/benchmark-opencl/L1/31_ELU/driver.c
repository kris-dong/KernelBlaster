...
static void elu_cpu(const cl_half* x, cl_half* y, int count, float alpha) {
    for (int i = 0; i < count; i++) {
        float val = half_to_float(x[i]);
        float result;
        if (val >= 0.0f) {
            result = val;
        } else {
            result = alpha * (expf(val) - 1.0f);
        }
...
int main(int argc, char** argv) {
    ...
    cl_half* h_y   = (cl_half*)malloc(total_bytes);
    cl_half* h_ref = (cl_half*)malloc(total_bytes);

    fill_random_half(h_x, (int)total_elems, 42);

    if (g_generate_reference) {
        printf("Computing CPU reference ELU (alpha=%.2f) for %d elements...\n", alpha, (int)total_elems);
        elu_cpu(h_x, h_ref, (int)total_elems, alpha);
    ...
    cl_kernel kernel = clCreateKernel(program, "elu_kernel", &err);
    ...
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: buffer d_x creation (%d)\n", err); return 1; }
    cl_mem d_y = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, total_bytes, NULL, &err);
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: buffer d_y creation (%d)\n", err); return 1; }

    int total_int = (int)total_elems;
    ...
    size_t global_work[1] = { (total_elems + 255) / 256 * 256 };
    ...
    if (g_profile) { print_event_timing("elu_kernel", event); clReleaseEvent(event); }

    err = clEnqueueReadBuffer(queue, d_y, CL_TRUE, 0, total_bytes, h_y, 0, NULL, NULL);
