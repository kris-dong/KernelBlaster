...
static void hardtanh_cpu(const cl_half* x, cl_half* out, int count) {
    for (int i = 0; i < count; i++) {
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for HardTanh, total=%d...\n", (int)total);
        hardtanh_cpu(h_x, h_ref, total);
    ...
    cl_kernel kernel = clCreateKernel(program, "hardtanh", &err);
    ...
    clSetKernelArg(kernel, 3, sizeof(float),  &min_val);
    clSetKernelArg(kernel, 4, sizeof(float),  &max_val);

    size_t global_work = (total + 255) / 256 * 256;
    ...
    if (g_profile) { print_event_timing("hardtanh", event); clReleaseEvent(event); }
