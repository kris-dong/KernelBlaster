...
#define DIM1 256
#define DIM2 256
#define REDUCE_DIM 1  // reducing over dim=1
...
// CPU reference: max reduction over dim=1
// Input: (BATCH_SIZE, DIM1, DIM2), Output: (BATCH_SIZE, DIM2)
static void max_reduce_dim1_cpu(const cl_half* x, cl_half* out) {
    for (int b = 0; b < BATCH_SIZE; b++) {
        for (int d2 = 0; d2 < DIM2; d2++) {
            ...
            out[b * DIM2 + d2] = float_to_half(max_val);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    // Output: (BATCH_SIZE, DIM2) - max over dim1
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for max reduction over dim=1...\n");
        max_reduce_dim1_cpu(h_x, h_ref);
        if (save_reference(h_ref, (int)output_elems, REFERENCE_FILE)) {
            printf("Reference saved to %s (%zu bytes)\n", REFERENCE_FILE, output_bytes);
            printf("passed\n");
        } else {
            printf("failed\n");
        }
    ...
    cl_kernel kernel = clCreateKernel(program, "max_reduce_dim1", &err);
    ...
    if (g_profile) { print_event_timing("max_reduce_dim1", event); clReleaseEvent(event); }
