...
static void rms_norm_cpu(const cl_half* x, cl_half* out,
                          int batch, int features, int d1, int d2, float eps) {
    // x shape: (batch, features, d1, d2)
    // For each (b, d1_idx, d2_idx), compute RMS across features
        ...
        for (int i = 0; i < d1; i++) {
            for (int j = 0; j < d2; j++) {
                ...
                for (int f = 0; f < features; f++) {
                    int idx = b * features * d1 * d2 + f * d1 * d2 + i * d2 + j;
                    float val = half_to_float(x[idx]);
                    mean_sq += val * val;
                }
...
int main(int argc, char** argv) {
    ...
    size_t total_elems = (size_t)BATCH_SIZE * FEATURES * DIM1 * DIM2;
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for RMSNorm...\n");
        rms_norm_cpu(h_x, h_ref, BATCH_SIZE, FEATURES, DIM1, DIM2, EPS);
    ...
    int ref_loaded = load_reference(h_ref, total_elems, REFERENCE_FILE);
    if (!ref_loaded) {
    ...
    clSetKernelArg(kernel, 2, sizeof(int), &num_features);
    clSetKernelArg(kernel, 3, sizeof(int), &dim1_val);
    clSetKernelArg(kernel, 4, sizeof(int), &dim2_val);
    clSetKernelArg(kernel, 5, sizeof(float), &eps_val);

    // Global: (DIM2, DIM1, BATCH_SIZE) -> each work item handles one (b, d1, d2)
    size_t global_work[3] = {DIM2, DIM1, BATCH_SIZE};
    ...
    if (g_profile) { print_event_timing("rms_norm", event); clReleaseEvent(event); }
