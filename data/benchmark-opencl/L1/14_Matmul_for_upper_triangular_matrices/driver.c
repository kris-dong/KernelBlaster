...
static float rand_normal(void) {
    float u1, u2;
    do { u1 = (float)rand() / (float)RAND_MAX; } while (u1 == 0.0f);
    u2 = (float)rand() / (float)RAND_MAX;
    return sqrtf(-2.0f * logf(u1)) * cosf(2.0f * 3.14159265358979323846f * u2);
}

/* Fill an upper triangular matrix with randn values (lower triangle = 0) */
static void fill_upper_triangular_randn(cl_half* data, int n, unsigned int seed) {
    srand(seed);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (j >= i) {
...
/* CPU reference: C = triu(A * B), where A and B are upper triangular */
static void matmul_triu_cpu(const cl_half* A, const cl_half* B, cl_half* C, int n) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (j < i) {
                /* Below diagonal: triu zeros these out */
                C[i * n + j] = float_to_half(0.0f);
            } else {
                /* For upper triangular result: only sum k from i to j
                 * since A[i][k]=0 for k<i and B[k][j]=0 for k>j */
                float sum = 0.0f;
                for (int k = i; k <= j; k++) {
...
int main(int argc, char** argv) {
    ...
    fill_upper_triangular_randn(h_A, N, 42);
    fill_upper_triangular_randn(h_B, N, 123);

    if (g_generate_reference) {
        printf("Computing CPU reference (fp16) for N=%d...\n", N);
        matmul_triu_cpu(h_A, h_B, h_ref, N);
    ...
    cl_kernel kernel = clCreateKernel(program, "matmul_triu", &err);
    ...
    if (g_profile) { print_event_timing("matmul_triu", event); clReleaseEvent(event); }
