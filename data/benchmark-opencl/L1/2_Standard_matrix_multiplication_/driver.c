...
/* Additional helper functions (not mandated verbatim) */
...
static void matmul_cpu_half(const cl_half* A, const cl_half* B, cl_half* C, int m, int k, int n) {
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            float sum = 0.0f;
            for (int t = 0; t < k; t++) {
                float a = half_to_float(A[i * k + t]);
                float b = half_to_float(B[t * n + j]);
                sum += a * b;
            }
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        ...
        matmul_cpu_half(h_A, h_B, h_ref, M, K, N);
    ...
    if (err != CL_SUCCESS || num_platforms == 0) { fprintf(stderr, "ERROR: No OpenCL platform found\n"); return 1; }

    cl_uint num_devices = 0;
    err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_GPU, 1, &device, &num_devices);
    if (err != CL_SUCCESS || num_devices == 0) {
        err = clGetDeviceIDs(platform, CL_DEVICE_TYPE_ALL, 1, &device, &num_devices);
        if (err != CL_SUCCESS || num_devices == 0) { fprintf(stderr, "ERROR: No OpenCL device found\n"); return 1; }
    }
    ...
    /* Kernel args order must match the kernel signature */
    int m_val = M;
    int k_val = K;
    int n_val = N;
    ...
    err |= clSetKernelArg(kernel, 5, sizeof(int), &n_val);
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: clSetKernelArg (%d)\n", err); goto cleanup; }

    size_t global_work[2] = { (size_t)N, (size_t)M }; /* (cols, rows) -> (x, y) */
...
cleanup:
    ...
    return (g_generate_reference ? 0 : (errors > 0 ? 1 : 0));
}
