...
/* A is (K, M), B is (K, N), C = A.T @ B => C is (M, N)
...
/* C = A.T @ B: A is (K, M), B is (K, N), C is (M, N)
   C[m*N_DIM + n] = sum_k A[k*M_DIM + m] * B[k*N_DIM + n]
*/
static void matmul_transA_cpu(const cl_half* A, const cl_half* B, cl_half* C) {
    for (int m = 0; m < M_DIM; m++) {
        for (int n = 0; n < N_DIM; n++) {
            float sum = 0.0f;
            for (int k = 0; k < K_DIM; k++) {
                sum += half_to_float(A[k * M_DIM + m]) * half_to_float(B[k * N_DIM + n]);
            }
            C[m * N_DIM + n] = float_to_half(sum);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    size_t a_elems = (size_t)K_DIM * M_DIM;
    ...
    if (g_generate_reference) {
        ...
        matmul_transA_cpu(h_A, h_B, h_ref);
    ...
    cl_kernel kernel = clCreateKernel(program, "matmul_transA", &err);
    ...
    /* global_work: (N_DIM, M_DIM) - col dimension first */
    size_t global_work[2] = {N_DIM, M_DIM};
    ...
    if (g_profile) { print_event_timing("matmul_transA", event); clReleaseEvent(event); }
