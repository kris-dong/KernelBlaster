...
/* CPU reference: C = A.T @ B.T
 * A: (K, M) stored row-major -> A[k][m] = A_buf[k * M + m]
 * B: (N, K) stored row-major -> B[n][k] = B_buf[n * K + k]
 * A.T: (M, K) -> A.T[m][k] = A_buf[k * M + m]
 * B.T: (K, N) -> B.T[k][n] = B_buf[n * K + k]
 * C[m][n] = sum_k A.T[m][k] * B.T[k][n]
 *          = sum_k A_buf[k * M + m] * B_buf[n * K + k]
 */
static void matmul_transposed_cpu(const cl_half* A, const cl_half* B, cl_half* C,
                                   int M, int K, int N) {
    for (int m = 0; m < M; m++) {
        for (int n = 0; n < N; n++) {
            float sum = 0.0f;
            for (int k = 0; k < K; k++) {
                sum += half_to_float(A[k * M + m]) * half_to_float(B[n * K + k]);
            }
            C[m * N + n] = float_to_half(sum);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    /* A: (K, M), B: (N, K), C: (M, N) */
    size_t A_elems = (size_t)DIM_K * DIM_M;
    size_t B_elems = (size_t)DIM_N * DIM_K;
    size_t C_elems = (size_t)DIM_M * DIM_N;
    ...
    fill_random_half(h_A, (int)A_elems, 42);
    fill_random_half(h_B, (int)B_elems, 123);

    if (g_generate_reference) {
        printf("Computing CPU reference (fp16) M=%d K=%d N=%d...\n", DIM_M, DIM_K, DIM_N);
        matmul_transposed_cpu(h_A, h_B, h_ref, DIM_M, DIM_K, DIM_N);
        if (save_reference(h_ref, (int)C_elems, REFERENCE_FILE)) {
    ...
    int ref_loaded = load_reference(h_ref, (int)C_elems, REFERENCE_FILE);
    if (!ref_loaded) {
        if (g_profile)
    ...
    cl_kernel kernel = clCreateKernel(program, "matmul_transposed", &err);
    ...
    clSetKernelArg(kernel, 3, sizeof(int), &dim_m);
    clSetKernelArg(kernel, 4, sizeof(int), &dim_k);
    clSetKernelArg(kernel, 5, sizeof(int), &dim_n);

    /* global: (N, M), local: (16, 16) */
    size_t global_work[2] = {DIM_N, DIM_M};
    ...
    if (g_profile) { print_event_timing("matmul_transposed", event); clReleaseEvent(event); }
