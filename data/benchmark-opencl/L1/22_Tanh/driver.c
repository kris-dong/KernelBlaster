...
static void tanh_cpu_half(const cl_half* x, cl_half* out, int count) {
    for (int i = 0; i < count; i++) {
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for batch=%d, dim=%d...\n", BATCH_SIZE, DIM);
        tanh_cpu_half(h_x, h_ref, total);
    ...
    cl_kernel kernel = clCreateKernel(program, "tanh_activation", &err);
    ...
    if (g_profile) { print_event_timing("tanh_activation", event); clReleaseEvent(event); }
