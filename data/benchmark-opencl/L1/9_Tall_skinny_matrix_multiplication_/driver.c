...
/* A is (M, K), B is (K, M_out) where:
 * M = 16384, K = 16, M_out = 16384
 * Output C is (M, M_out) = (16384, 16384)
 */
#define M_DIM   16384
#define K_DIM   16
 ...
 * A: (M_DIM, K_DIM), B: (K_DIM, OUT_DIM), C: (M_DIM, OUT_DIM) */
static void matmul_cpu(const cl_half* A, const cl_half* B, cl_half* C) {
    for (int i = 0; i < M_DIM; i++) {
        for (int j = 0; j < OUT_DIM; j++) {
            float sum = 0.0f;
            for (int k = 0; k < K_DIM; k++) {
                sum += half_to_float(A[i * K_DIM + k]) * half_to_float(B[k * OUT_DIM + j]);
            }
            C[i * OUT_DIM + j] = float_to_half(sum);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    size_t b_elems = (size_t)K_DIM * OUT_DIM;
    size_t c_elems = (size_t)M_DIM * OUT_DIM;
    ...
    cl_kernel kernel = clCreateKernel(program, "tall_skinny_matmul", &err);
    ...
    /* global: (OUT_DIM, M_DIM) = (16384, 16384), local: (16, 16) */
    size_t global_work[2] = {OUT_DIM, M_DIM};
    ...
    if (g_profile) { print_event_timing("tall_skinny_matmul", event); clReleaseEvent(event); }
