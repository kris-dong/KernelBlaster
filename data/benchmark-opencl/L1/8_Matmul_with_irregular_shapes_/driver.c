...
static void matmul_cpu_half(const cl_half* A, const cl_half* B, cl_half* C,
                             int m, int k, int n) {
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            float sum = 0.0f;
            for (int p = 0; p < k; p++) {
                sum += half_to_float(A[i * k + p]) * half_to_float(B[p * n + j]);
            }
...
int main(int argc, char** argv) {
    ...
    size_t a_elems = (size_t)M_DIM * K_DIM;
    size_t b_elems = (size_t)K_DIM * N_DIM;
    size_t c_elems = (size_t)M_DIM * N_DIM;
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference (fp16) for M=%d K=%d N=%d...\n", M_DIM, K_DIM, N_DIM);
        matmul_cpu_half(h_A, h_B, h_ref, M_DIM, K_DIM, N_DIM);
    ...
    cl_kernel kernel = clCreateKernel(program, "matmul_irregular", &err);
    ...
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: d_A creation (%d)\n", err); return 1; }
    cl_mem d_B = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, b_bytes, h_B, &err);
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: d_B creation (%d)\n", err); return 1; }
    cl_mem d_C = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, c_bytes, NULL, &err);
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: d_C creation (%d)\n", err); return 1; }
    ...
    clSetKernelArg(kernel, 4, sizeof(int), &k_val);
    clSetKernelArg(kernel, 5, sizeof(int), &n_val);

    /* Round up global work sizes to multiples of local work size */
    size_t local_w = 16;
    size_t local_h = 16;
    size_t global_cols = ((N_DIM + local_w - 1) / local_w) * local_w;
    size_t global_rows = ((M_DIM + local_h - 1) / local_h) * local_h;
    size_t global_work[2] = {global_cols, global_rows};
    size_t local_work[2]  = {local_w, local_h};
    ...
    if (g_profile) { print_event_timing("matmul_irregular", event); clReleaseEvent(event); }
