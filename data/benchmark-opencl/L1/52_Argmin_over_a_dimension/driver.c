...
/* argmin along dim=1 for shape (BATCH_SIZE, DIM1, DIM2) -> (BATCH_SIZE, DIM2) */
static void argmin_cpu(const cl_half* x, int* out) {
...
int main(int argc, char** argv) {
    ...
    if (g_generate_reference) {
        printf("Computing CPU reference for argmin...\n");
        argmin_cpu(h_x, h_ref);
    ...
    cl_kernel kernel = clCreateKernel(program, "argmin_dim1", &err);
    ...
    if (err != CL_SUCCESS) { fprintf(stderr, "ERROR: enqueue warmup (%d)\n", err); return 1; }
    clFinish(queue);
    ...
    if (g_profile) { print_event_timing("argmin_dim1", event); clReleaseEvent(event); }
